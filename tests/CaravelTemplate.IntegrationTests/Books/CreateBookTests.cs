...
public class CreateBookTests(TestingWebApplicationFactory factory) : BaseIntegrationTest(factory)
{
    ...
    {
        var bookRequest = new CreateBookRequest("", "My Testing Book");
    ...
    {
        var bookRequest = new CreateBookRequest("My Book", "My Testing Book");
        var httpClient = TestServer.CreateClient();
        ...
        var bookResponse = await response.Content.ReadFromJsonAsync<CreateBookResponse>();
